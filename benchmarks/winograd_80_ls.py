"""
Winograd 80% with Light-Speed constraints.
Identical to winograd_80.py except score_from_vecs uses
forward_with_light_speed instead of plain forward().
epochs=40, lr=3e-4
"""
...
def build_pronoun_augmentation():
    ...
    for sent, correct, wrong in (
        HE_MALE   + HE_EXTRA[:10] +
        SHE_FEMALE + SHE_EXTRA[:10] +
        IT_OBJECT  + IT_BALANCED +
        THEY_BALANCED
    ):
...
class WinogradSpringModel(nn.Module):
    ...
    def score_from_vecs(self, sentence: str,
        ...
        Uses light speed forward with fallback to plain forward.
        """
        ...
        # Use light speed forward instead of regular forward
        try:
...
def train(model, train_pairs, epochs=40):
    optimizer = AdamW(
    ...
    for epoch in range(epochs):
        ...
        if acc > best_train_acc:
            ...
            }, 'checkpoints/winograd80_ls_best.pt')
...
def evaluate(model, test_schemas, verbose=True):
    ...
    print(f"BERT target: ~70%   80% target: 80%   Previous best: 80.7%")
...
if __name__ == "__main__":
    print("=" * 60)
    print("Winograd 80% — MiniLM + Solar Spring + Light Speed")
    print("=" * 60)
    ...
    print(f"Previous best: 80.7%  |  This run: {full_acc:.1f}%")
    if full_acc > 80.7:
        print("IMPROVEMENT CONFIRMED")
    else:
        print(f"Delta: {full_acc - 80.7:+.1f}%")
